...
// Create floors
const floorCont3Geo = new THREE.PlaneGeometry(190, 190);
const floorCont3Mat = new THREE.MeshStandardMaterial({
    color: 0x78BE21,
    side: THREE.DoubleSide,
    wireframe: true
});

const floorCont3PhysMat = new CANNON.Material();
const floorCont3Mesh = new THREE.Mesh(floorCont3Geo, floorCont3Mat);
scene.add(floorCont3Mesh);

// Physics floor
const floorCont3Shape = new CANNON.Box(new CANNON.Vec3(95, 95, 1)); // Half of your desired dimensions
const floorCont3Body = new CANNON.Body({
    shape: floorCont3Shape,
    type: CANNON.Body.STATIC,
    material: floorCont3PhysMat,
});
floorCont3Body.position.copy( new CANNON.Vec3(blockWidth/2 - 10,5,-10));
floorCont3Body.quaternion.setFromEuler(rotationAngle, 0, 0);
world.addBody(floorCont3Body);

floorCont3Mesh.position.copy(floorCont3Body.position);
